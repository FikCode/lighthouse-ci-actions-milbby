...
[[ -n "$INPUT_PRODUCT_HANDLE_2" ]]    && export SHOP_PRODUCT_HANDLE_2="$INPUT_PRODUCT_HANDLE_2"
[[ -n "$INPUT_PRODUCT_HANDLE_3" ]]    && export SHOP_PRODUCT_HANDLE_3="$INPUT_PRODUCT_HANDLE_3"
[[ -n "$INPUT_PRODUCT_HANDLE_4" ]]    && export SHOP_PRODUCT_HANDLE_4="$INPUT_PRODUCT_HANDLE_4"
...
if [[ -n "${SHOP_PRODUCT_HANDLE_2+x}" ]]; then
  ...
  product_handle_2="$(echo "$product_response" | jq -r '.products[1].handle')"
  log "Using $product_handle_2"
fi
...
if [[ -n "${SHOP_PRODUCT_HANDLE_3+x}" ]]; then
  ...
  product_response="$(api_request "$host/admin/api/2021-04/products.json?published_status=published&limit=4")"
  product_handle_3="$(echo "$product_response" | jq -r '.products[2].handle')"
  log "Using $product_handle_3"
fi
...
if [[ -n "${SHOP_PRODUCT_HANDLE_4+x}" ]]; then
  ...
  product_response="$(api_request "$host/admin/api/2021-04/products.json?published_status=published&limit=4")"
  product_handle_4="$(echo "$product_response" | jq -r '.products[3].handle')"
  log "Using $product_handle_4"
fi
...
if [[ -n "${SHOP_COLLECTION_HANDLE_2+x}" ]]; then
  ...
  collection_handle_2="$(echo "$collection_response" | jq -r '.custom_collections[1].handle')"
  log "Using $collection_handle"
fi


if [[ -n "${SHOP_SEARCH_STRING+x}" ]]; then
  search_string="$SHOP_SEARCH_STRING"
else
  log "Setting up search string"
  search_string="&q=cricut&options[prefix]=last"
  log "Using $search_string"
fi
...
ci:
  collect:
    url:
      ...
      - "$host/products/$product_handle_2$query_string"
      - "$host/products/$product_handle_3$query_string"
      - "$host/products/$product_handle_4$query_string"
      - "$host/collections/$collection_handle$query_string"
      - "$host/collections/$collection_handle_2$query_string"
      - "$host/search/$query_string$search_string"
